function [leftBuyer,leftSeller] = SurvivalRule(t,Buyer,Seller,f_I,f_J,x)
%% input 
...
% x:      List, Quantity allocated to each shipper
%% output
...
for i = 1:buyerNumber
    ...
    if t < buyerDepaturetime(i) && f_I(i) ~= 0 && f_I(i)<buyerValue(i) && x(i) < buyerDemand(i) 
       buyerDemand(i) = buyerDemand(i)-x(i);
        leftBuyerset = [leftBuyerset,i];
        buyerLanematrix(i,buyerLane(i)) = buyerDemand(i);
    end
end
...
if isempty(leftBuyerset)
    ...
    for i = 1:leftBuyer.number
        ...
        leftBuyer.p_i(i) = max(f_I(leftBuyerset(i)),p_i(leftBuyerset(i)));

...
for bb = 2
    for aa = 30
        global M K laneNumber sellerBundlenumber bundle S
        ...
        for i = 1:t
            ...
            [Allocation(i).x,Allocation(i).y,Pricing(i).payment,Pricing(i).revenue,f_I,f_J, ...
                Result(i).buyerUtility,Result(i).sellerUtility,Result(i).platformPayoff,Result(i).socialWelfare_realized,Result(i).allocation] = PricingAllocationRlue(activeBuyer_singlePeriod,activeSeller_singlePeriod);
            
            [leftBuyer,leftSeller] = SurvivalRule(i,activeBuyer_singlePeriod,activeSeller_singlePeriod,f_I,f_J,Allocation(i).x);

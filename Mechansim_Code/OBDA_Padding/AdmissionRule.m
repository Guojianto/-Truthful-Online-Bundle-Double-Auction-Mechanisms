...
for i = 1:buyerNumber
    if buyerDepaturetime(i) - buyerArrivaltime(i) == K || buyerArrivaltime(i) == 1
        ...
        for j = max(buyerDepaturetime(i) - K,1): (buyerArrivaltime(i) - 1)
            ...
            [p_I,p_J] = StaticPricingMethod_Gurobi(pastBuyer,activeSeller(j),S);
...
for i = 1:sellerNumber
    if sellerDepaturetime(i) - sellerArrivaltime(i) == K || sellerArrivaltime(i) == 1
        ...
        for j = max(sellerDepaturetime(i) - K,1): (sellerArrivaltime(i) - 1)
            ...
            [p_I,p_J] = StaticPricingMethod_Gurobi(activeBuyer(j),pastSeller,S);
...
if ~isempty(leftBuyer)
    ...
    ActiveBuyer.value = [leftBuyer.value;ActiveBuyer.value] ;
    ActiveBuyer.lane = [leftBuyer.lane;ActiveBuyer.lane];
    ActiveBuyer.demand = [leftBuyer.demand;ActiveBuyer.demand];
    ActiveBuyer.laneMatrix = [leftBuyer.laneMatrix;ActiveBuyer.laneMatrix];
    ActiveBuyer.p_i = [leftBuyer.p_i;ActiveBuyer.p_i];
    ActiveBuyer.depatureTime = [leftBuyer.depatureTime;ActiveBuyer.depatureTime];
    ActiveBuyer.arrivalTime=  [leftBuyer.arrivalTime;ActiveBuyer.arrivalTime];
end
